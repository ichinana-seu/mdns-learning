...
service_mdns(const char* hostname, const char* service_name, int service_port) {
	...
	service.txt_record[0] = (mdns_record_t){.name = service.service_instance,
	                                        ...
	                                        .data.txt.key = {MDNS_STRING_CONST("mdns-reg")},
	                                        .data.txt.value = {MDNS_STRING_CONST("1.0.1")},
	                                        .rclass = 0,
	                                        .ttl = 0};
...
main(int argc, const char* const* argv) {
	printf("mDNS service register ("PROGRAM_NAME")\n");
	...
	for (int iarg = 0; iarg < argc; ++iarg) {
		if ((strcmp(argv[iarg], "--help") == 0) || (strcmp(argv[iarg], "-h") == 0)) {
			printf(" |    Function:   Register and broadcast your service to the network. (1.0.1)\n");
			printf(" |   Mechanism:   Register \"instancename._servicename._tcp.local\" and reply request from other devices.\n");
			...
			printf(" |     Example:   \""PROGRAM_NAME" _ipp._tcp.local. myprinter 631\"\n");
			return 1;
		}
	}
	if (argc==1 || argc>4){
