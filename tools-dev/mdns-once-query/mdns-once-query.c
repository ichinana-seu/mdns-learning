#define PROGRAM_NAME "mdns-once-query"
...
main(int argc, const char* const* argv) {
	printf("mDNS query once ("PROGRAM_NAME")\n");
	...
	for (int iarg = 0; iarg < argc; ++iarg) {
		if ((strcmp(argv[iarg], "--help") == 0) || (strcmp(argv[iarg], "-h") == 0) ) {
			...
			printf(" |       Usage:   \""PROGRAM_NAME" <service_name._tcp.domain>\"\n");
			printf(" |     Example:   \""PROGRAM_NAME" _airplay._tcp.local.\" to acquire details of all Apple Airplay devices.\n");
			return 1;
		}
	}
	if (argc==1 || argc>2){
