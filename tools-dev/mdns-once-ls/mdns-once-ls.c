#define PROGRAM_NAME "mdns-once-ls"
...
main(int argc, const char* const* argv) {
	printf("mDNS service discover once ("PROGRAM_NAME")\n");
	...
	for (int iarg = 0; iarg < argc; ++iarg) {
		if ((strcmp(argv[iarg], "--help") == 0) || (strcmp(argv[iarg], "-h") == 0)) {
			printf(" |    Function:   Discovery all public-registered services in the network. (1.0.1)\n");
			printf(" |   Mechanism:   Search \"_services._dns-sd._udp.local\" and receive PTR records from the network.\n");
			printf(" |       Usage:   \""PROGRAM_NAME"\" simply with no option.\n");
			return 1;
		}
	}
	if (argc > 1){
